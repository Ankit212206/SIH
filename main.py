...
def sensor_reading_from(document):
    return [
        document.get("gas_level_ppm", document.get("gas", 30)),
        document.get("dust", 650),
        document.get("temperature_c", document.get("temp", document.get("Temp", 30))),
        document.get("humidity_pct", document.get("humid", document.get("Humid", 40))),
    ]
...
def get_data():
    ...
    try:
        # Use the normalized timestamp created by dbReceive, rather than the
        # ObjectId creation time, so the dashboard follows telemetry time.
        documents = list(collection.find({}, sort=[("timestamp_sort", -1), ("_id", -1)]).limit(16))
        documents.reverse()
        ...
        for document in documents:
            history.append({
                ...
                "temperature_c": document.get("temperature_c", document.get("temp", document.get("Temp"))),
                "humidity_pct": document.get("humidity_pct", document.get("humid", document.get("Humid"))),
                "gas_level_ppm": document.get("gas_level_ppm", document.get("gas")),
